...
from models.resnet import vgg11, CompTaskModel 
from models.query_models import LossNet, DecoderVGG, FDALDiscriminator
from train_test import train, test
from load_dataset import load_dataset
from selection_methods import query_samples
from config import *
from models.teacher import VGG19_Teacher, CommonFE, VGG, TaskModel
                    ...
                    #   decoder = Decoder().cuda()
                      sampler = FDALDiscriminator().cuda()
                        ...
                        if args.dataset == 'cifar10':
                            teacher = VGG19_Teacher(NO_CLASSES).cuda()
                            teacher.load_state_dict(torch.load(TEACHER_PATH_C10, map_location='cuda'))
                            teacher.eval()
                        if args.dataset == 'cifar100':
                            teacher = VGG19_Teacher(NO_CLASSES).cuda()
                            teacher.load_state_dict(torch.load(TEACHER_PATH_C100, map_location='cuda'))
                            teacher.eval()
                    decoder = DecoderVGG().cuda()
                    sampler = FDALDiscriminator().cuda()
