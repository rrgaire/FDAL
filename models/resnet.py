...
class VGG(nn.Module):
    ...
    def forward(self, x):
        ...
        out4 = feats.view(feats.size(0), -1)
        x = self.classifier(out4)
        return x, out4, [feat[0], feat[1], feat[2], feat[3], feats]
...
class CompTaskModel(nn.Module):
    def __init__(self, num_classes):
        super(CompTaskModel, self).__init__()
        self.feature_extractor = CompVGGFeature()
        self.classifier = CompVGGClassifier(num_classes)

    def forward(self, x):
        feat = self.feature_extractor(x)
        ffeat, x = self.classifier(feat[-1])
        return x, ffeat, feat
...
class CompVGGClassifier(nn.Module):
    ...
    def forward(self, x):
        x = self.avgpool(x)
        ffeat = torch.flatten(x, 1)
        x = self.classifier(ffeat)
        return ffeat, x
