...
# Custom
from config import *
from models.query_models import VAE, TAVAE, TADiscriminator, Discriminator, GCN
...
def train_tavaal(models, optimizers, labeled_dataloader, unlabeled_dataloader, cycle):
...
def query_samples(model, method, data_unlabeled, subset, labeled_set, cycle, args):
    ...
    if method == 'TA-VAAL':
        ...
        if args.dataset == 'fashionmnist':
            vae = TAVAE(28,1,3)
            discriminator = TADiscriminator(28)
        else:
            vae = TAVAE()
            discriminator = TADiscriminator(32)
        ...
        train_tavaal(models,optimizers, labeled_loader, unlabeled_loader, cycle+1)
        task_model = models['backbone']
        ranker = models['module']        
        all_preds, all_indices = [], []
    ...
    if method == 'FDAL':
        ...
        unlabeled_sampler = SubsetSequentialSampler(subset)
        ...
        for images, _, indices in unlabeled_dataloader:
            ...
            with torch.no_grad():
                _, _, features = model['backbone'](images)
                # features = model['decoder'](features[4])

                preds = model['sampler'](features[3])
