...
class TAVAE(nn.Module):
    """Encoder-Decoder architecture for both WAE-MMD and WAE-GAN."""
    def __init__(self, z_dim=32, nc=3, f_filt=4):
        super(TAVAE, self).__init__()
        self.z_dim = z_dim
        self.nc = nc
        self.f_filt = f_filt
        self.encoder = nn.Sequential(
...
class TADiscriminator(nn.Module):
    """Adversary architecture(Discriminator) for WAE-GAN."""
    def __init__(self, z_dim=10):
        super(TADiscriminator, self).__init__()
        self.z_dim = z_dim
        self.net = nn.Sequential(
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(in_channels=1024, out_channels=2048, kernel_size=3, padding=1)
...
class FDALDiscriminator(nn.Module):
    def __init__(self):
        super(FDALDiscriminator, self).__init__()
        self.conv = nn.Conv2d(in_channels=2048, out_channels=2048, kernel_size=3, padding=1)
