...
def train_epoch(models, method, criterion, optimizers, dataloaders, epoch, epoch_loss):
    ...
    for data in tqdm(dataloaders['train'], leave=False, total=len(dataloaders['train'])):
        ...
        # print(torch.sum(target_loss) / target_loss.size(0) )
        ...
        if method == 'lloss' or method == 'TA-VAAL':
            ...
            # print(teach_lab_feats, features[4])
            dec_lab_feats = models['decoder'](features[3])
            # print(dec_lab_feats.max(), teach_lab_feats.max())
            
            # (F.log_softmax(dec_lab_feats, dim=1), F.softmax(teach_lab_feats, dim=1))
            feat_loss = criterion['mse_loss'](dec_lab_feats, teach_lab_feats)
            logit_loss = criterion['mse_loss'](scores, teach_lab_logits)

            loss = m_backbone_loss + FEAT_WT * feat_loss + LOGIT_WT * logit_loss

            # print(loss.item() , m_backbone_loss.item(), feat_loss.item(), logit_loss.item())
            if epoch >= EPOCH_FDAL:
                disc_lab = models['sampler'](features[3]).squeeze(1)

                _, _, unlab_feats = models['backbone'](unlab_inputs) 
                # dec_unlab_feats = models['decoder'](unlab_feats)
                disc_unlab = models['sampler'](unlab_feats[3]).squeeze(1)
            ...
            if epoch >= EPOCH_FDAL:
                ...
                with torch.no_grad():
                    ...
                    _, _, unlab_feats = models['backbone'](unlab_inputs) 
                    # unlab_feats = models['decoder'](unlab_feats[4]) 
                
                labeled_preds = models['sampler'](lab_feats[3]).squeeze(1)
                unlabeled_preds = models['sampler'](unlab_feats[3]).squeeze(1)
                ...
                disc_loss = ( criterion['bce_loss'](labeled_preds, lab_real_preds) + criterion['bce_loss'](unlabeled_preds, unlab_fake_preds) ) / 2
